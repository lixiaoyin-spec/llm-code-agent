...
| --- | --- | --- | --- | --- |
| 1 开场 | 0:00-0:08 | 终端输入 nihue 启动，logo 与全宽输入栏出现 | 一句话介绍 Nihue：自研编程智能体，基于 DeepSeek deepseek-v4-pro | CLI 入口、logo、彩色 UI、模型与工作目录展示 |
...
- 旁白："这是 Nihue，我独立实现的编程智能体。它基于 DeepSeek deepseek-v4-pro，能自主读写文件、执行命令完成编程任务。"
...
- 用真实 API 预演一遍镜头 3（deepseek-v4-pro）：
```powershell
python agent.py "阅读 TASK.md，按规格从零实现网页端井字棋游戏，补充单元测试并运行到全绿" --workspace demo/tic_tac_toe --auto-approve --max-turns 60
```
